...
export default function AlertsPage() {
  ...
  const allFilteredAlerts = alerts.filter(
    (a) =>
      (symbolFilter === "all" || a.symbol === symbolFilter) && a.name.toLowerCase().includes(searchQuery.toLowerCase()),
  )
      ...
      >
        <CardContent className="p-6">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => onViewChart(alert.id)}>📊 View Chart</DropdownMenuItem>
                <DropdownMenuItem onClick={() => onEdit(alert.id)}>✏️ Edit Alert</DropdownMenuItem>
                {alert.status === "active" && (
                  <>
                    <DropdownMenuItem onClick={() => onPause(alert.id)}>⏸️ Pause Alert</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => onMute(alert.id)}>🔕 Mute for 1h</DropdownMenuItem>
                  </>
                )}
                {alert.status === "paused" && (
                  <DropdownMenuItem onClick={() => onResume(alert.id)}>▶️ Resume Alert</DropdownMenuItem>
                )}
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              <div className="text-xs uppercase text-gray-500 font-semibold mb-1">Target</div>
              <div className="text-lg font-semibold text-green-600 mb-1">{alert.targetLine}</div>
              ...
              <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 text-xs">{alert.alertType}</Badge>
            </div>

            {/* Right Column - Current Status */}
            <div>
              <div className="text-xs uppercase text-gray-500 font-semibold mb-1">Current Price</div>
              <div className="text-2xl font-bold text-gray-900 mb-1">
                $
              ...
              <div className={`text-lg mb-3 ${alert.direction === "toward" ? "text-orange-600" : "text-gray-600"}`}>
              ...
              <div className="mb-2">
                <div className="text-xs text-gray-500 mb-1 flex justify-between items-center">
                  ...
                  {Math.abs(alert.distanceToTargetPercent) < 0.2 && (
                    <Badge className="bg-red-100 text-red-800 hover:bg-red-100 text-xs">⚠️ NEAR</Badge>
                  )}
                </div>
          ...
          {alert.status === "triggered" && alert.triggeredAt && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4">
              <div className="text-lg font-semibold mb-1">Triggered at: ${alert.triggeredPrice?.toLocaleString()}</div>
              <div className="text-sm text-gray-600 mb-2">Time: {alert.triggeredAt}</div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-200">
            <div className="flex items-center gap-4">
              <div className="flex gap-2 text-sm">
                <span className={alert.notifications.email ? "opacity-100" : "opacity-30"}>✉️ Email</span>
                <span className={alert.notifications.push ? "opacity-100" : "opacity-30"}>📱 Push</span>
                <span className={alert.notifications.sms ? "opacity-100" : "opacity-30"}>💬 SMS</span>
              </div>
              <span className="text-xs text-gray-500">Updated {alert.lastChecked}</span>
            </div>

            <div className="flex flex-wrap gap-2">
              <Button onClick={() => onViewChart(alert.id)} className="bg-blue-600 hover:bg-blue-700 text-white">
                📊 View Chart
              </Button>
              {alert.status === "active" && (
                <>
              ...
              {alert.status === "paused" && (
                <>
                  <Button onClick={() => onResume(alert.id)} className="bg-green-600 hover:bg-green-700 text-white">
                    ▶️ Resume
                  </Button>
                  <Button
                    onClick={() => onDelete(alert)}
          ...
          {alert.status === "triggered" && alert.autoArchiveIn && (
            <div className="text-xs text-gray-500 mt-2 text-right">Will be archived in {alert.autoArchiveIn}</div>
          )}
        </CardContent>
      </Card>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="border-l-4 border-l-green-500 shadow-md">
            <CardContent className="p-6">
              ...
              <div className="text-sm uppercase font-semibold text-gray-600 mb-2">Active</div>
          ...
          <Card className="border-l-4 border-l-gray-300 shadow-md">
            <CardContent className="p-6">
              ...
              <div className="text-sm uppercase font-semibold text-gray-600 mb-2">Paused</div>
              <div className="text-4xl font-bold text-gray-600 mb-1">{pausedAlerts.length}</div>
          ...
          <Card className="border-l-4 border-l-orange-500 shadow-md">
            <CardContent className="p-6">
              <div className="text-4xl mb-2">✅</div>
              <div className="text-sm uppercase font-semibold text-gray-600 mb-2">Triggered</div>
              <div className="text-4xl font-bold text-orange-600 mb-1">{triggeredAlerts.length}</div>
        ...
        {selectedAlerts.length > 0 && (
          <div className="bg-blue-600 text-white rounded-lg p-4 shadow-lg mb-6 flex flex-wrap items-center justify-between gap-4">
            <span className="font-semibold">{selectedAlerts.length} alerts selected</span>
            <div className="flex gap-2">
              <Button onClick={pauseSelected} variant="secondary" className="bg-white text-blue-600 hover:bg-gray-100">
                Pause Selected
              </Button>
              <Button onClick={deleteSelected} variant="secondary" className="bg-white text-blue-600 hover:bg-gray-100">
                Delete Selected
              </Button>
              <Button
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList>
                ...
                <TabsTrigger value="all">All ({allFilteredAlerts.length})</TabsTrigger>
              </TabsList>
            </Tabs>
        ...
        {activeTab === "active" && (
          <div>
            <h2 className="text-2xl font-bold mb-6">🟢 Active Alerts ({activeAlerts.length}):</h2>
            {activeAlerts.length === 0 ? (
              <Card className="bg-gray-50 border-2 border-dashed border-gray-300">
                <CardContent className="p-16 text-center">
                  <div className="text-8xl opacity-30 mb-4">🔔</div>
                  <h3 className="text-2xl text-gray-500 mb-2">No active alerts</h3>
                  <p className="text-gray-400 mb-6">Create your first alert to get notified about price movements</p>
                  <Button onClick={onCreate} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 text-lg">
        ...
        {activeTab === "paused" && (
          <div>
            <h2 className="text-2xl font-bold mb-6">⏸️ Paused Alerts ({pausedAlerts.length}):</h2>
        ...
        {activeTab === "triggered" && (
          <div>
            <h2 className="text-2xl font-bold mb-6">✅ Triggered Alerts (Last 7 days) ({triggeredAlerts.length}):</h2>
            {triggeredAlerts.length === 0 ? (
              <p className="text-gray-500 italic">No alerts have triggered in the last 7 days</p>
            ) : (
          ...
          <div>
            {allFilteredAlerts.length === 0 ? (
              ...
              <>
                {activeAlerts.length > 0 && (
                  <>
                    <h2 className="text-2xl font-bold mb-6 mt-8">🟢 Active Alerts ({activeAlerts.length}):</h2>
                    {activeAlerts.map(renderAlertCard)}
                  </>
                )}
                {pausedAlerts.length > 0 && (
                  <>
                    <h2 className="text-2xl font-bold mb-6 mt-12">⏸️ Paused Alerts ({pausedAlerts.length}):</h2>
                    {pausedAlerts.map(renderAlertCard)}
                  </>
                )}
                {triggeredAlerts.length > 0 && (
      ...
      <Dialog open={deleteModalOpen} onOpenChange={setDeleteModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-red-600">⚠️ Delete Alert?</DialogTitle>
          ...
          <div className="flex items-center gap-2 py-4">
            ...
            />
            <label htmlFor="dontAsk" className="text-sm text-gray-600 cursor-pointer">
              Don&apos;t ask me again
            </label>
          </div>
          <DialogFooter>
            ...
            <Button onClick={confirmDelete} className="bg-red-600 hover:bg-red-700 text-white px-6 py-3">
              Delete Alert
            </Button>
          </DialogFooter>
        </DialogContent>
