...
export default function TradingChart({
  ...
  const [nextUpdate, setNextUpdate] = useState<number>(tier === "PRO" ? 30 : 60)
  ...
  const generateMockData = useCallback(() => {
    ...
    const volatility = symbol === "BTCUSD" ? 100 : symbol === "XAUUSD" ? 5 : 0.002
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-4 rounded-lg border shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={timeframe} onValueChange={(v) => onTimeframeChange(v as Timeframe)}>
            ...
            <SelectContent>
              {TIMEFRAMES.map((t) => (
                <SelectItem key={t.value} value={t.value} disabled={tier === "FREE" && t.locked}>
                  <div className="flex items-center justify-between w-full gap-2">
                    <span>{t.label}</span>
                    {tier === "FREE" && t.locked && <Lock className="w-3 h-3 opacity-50" />}
                  </div>
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="text-xs text-muted-foreground text-right">
          <div>Last updated: {Math.floor((new Date().getTime() - lastUpdated.getTime()) / 1000)}s ago</div>
          <div className="font-medium text-primary">Next update: {nextUpdate}s</div>
        </div>
      </div>
          ...
          <CardContent>
            <div className="flex items-baseline justify-between">
              <span className="text-2xl font-bold">
                {symbol === "XAUUSD" ? "$2,656.79" : symbol === "BTCUSD" ? "$65,420.50" : "1.0850"}
              </span>
              <span className="text-xs text-green-600">+$4.70 (+0.18%)</span>
            </div>
            <div className="text-xs text-muted-foreground mt-1">5 touches | 8.5° | 120 bars</div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-medium text-emerald-500">Horizontal Support (B-B1)</CardTitle>
              <span className="bg-orange-100 text-orange-700 text-[10px] px-1.5 py-0.5 rounded font-medium">NEAR</span>
            </div>
          </CardHeader>
          <CardContent>
            <div className="flex items-baseline justify-between">
              <span className="text-2xl font-bold">
                {symbol === "XAUUSD" ? "$2,646.59" : symbol === "BTCUSD" ? "$64,850.20" : "1.0780"}
              </span>
              <span className="text-xs text-red-600">-$5.50 (-0.21%)</span>
            </div>
            <div className="text-xs text-muted-foreground mt-1">4 touches | -1.2° | 100 bars</div>
