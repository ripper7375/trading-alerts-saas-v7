...
import { ALL_SYMBOLS, FREE_SYMBOLS, SYMBOL_CATEGORIES, type UserTier, type Symbol } from "@/lib/chart-data"
...
export function SymbolSelector({ userTier, selectedSymbol, onSymbolChange, onUpgradeClick }: SymbolSelectorProps) {
  ...
  const filteredSymbols = useMemo(() => {
    ...
    return ALL_SYMBOLS.filter(
      (symbol) => symbol.code.toLowerCase().includes(query) || symbol.name.toLowerCase().includes(query),
    )
  }, [searchQuery])
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-2 flex-1">
          <span className="text-lg font-bold text-gray-900">{selectedSymbolData?.code || selectedSymbol}</span>
          <span className="text-sm text-gray-600 hidden sm:inline">{selectedSymbolData?.name}</span>
        </div>
          ...
          <div className="absolute top-full mt-2 w-[95vw] sm:w-96 bg-white rounded-xl shadow-2xl border-2 border-gray-200 max-h-96 overflow-auto z-50 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="py-2">
              {Object.entries(groupedSymbols).map(([category, symbols]) => {
                ...
                const categoryInfo = SYMBOL_CATEGORIES[category as keyof typeof SYMBOL_CATEGORIES]
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-base font-semibold text-gray-900">{symbol.code}</span>
                              {isLocked && (
                          ...
                          <div className="flex flex-col items-end ml-2 shrink-0">
                            <div className="text-sm font-medium text-gray-700 hidden sm:block">{symbol.price}</div>
                            <div
                              className={cn(
                                "text-xs flex items-center gap-0.5",
                                symbol.isPositive ? "text-green-600" : "text-red-600",
                              )}
                            >
                              {symbol.isPositive ? (
                              ...
                              <span className="hidden sm:inline">{symbol.change}</span>
