...
import { ALL_TIMEFRAMES, FREE_TIMEFRAMES, type UserTier } from "@/lib/chart-data"
...
export function TimeframeSelector({
  ...
  const selectedTimeframeData = ALL_TIMEFRAMES.find((t) => t.code === selectedTimeframe)
  ...
  const handleTimeframeSelect = (timeframeCode: string) => {
    if (userTier === "FREE" && !FREE_TIMEFRAMES.includes(timeframeCode)) {
      ...
      onUpgradeClick("timeframe", `${timeframe?.code} (${timeframe?.name})` || timeframeCode)
      return
    }
    ...
    <div className="relative">
      ...
      <button
        ...
        className={cn(
          ...
          isOpen ? "border-blue-600 bg-blue-50" : "border-gray-300 hover:border-blue-600",
        )}
      >
        <div className="flex items-center gap-2 flex-1">
          <span className="text-lg font-bold text-gray-900">{selectedTimeframeData?.code || selectedTimeframe}</span>
          <span className="text-sm text-gray-600 hidden sm:inline">{selectedTimeframeData?.name}</span>
        </div>
          ...
          <div className="absolute top-full mt-2 w-72 bg-white rounded-xl shadow-2xl border-2 border-gray-200 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected && "border-blue-600 bg-blue-50 text-blue-600 font-semibold",
                      !isSelected && !isLocked && "border-gray-200 hover:border-blue-400 hover:bg-blue-50",
                  ...
                  >
                    ...
                    <div className="text-xl font-bold text-gray-900 mb-1">{timeframe.code}</div>
                    <div className="text-xs text-gray-600">{timeframe.name}</div>
                    {isLocked && <Lock className="w-4 h-4 text-gray-400 mx-auto mt-1" />}
                  </button>
                )
              })}
            </div>
            ...
            {userTier === "FREE" && (
              <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg p-4 m-4 mt-0">
                ...
                <div className="text-sm font-semibold mb-1">Unlock 6 more timeframes</div>
                <div className="text-xs opacity-90 mb-3">Upgrade to PRO for M5-H12</div>
                <button
                  onClick={() => onUpgradeClick("timeframe", "additional timeframes")}
