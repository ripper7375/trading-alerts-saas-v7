...
export default function DashboardHome({ userName, userTier, stats, watchlistItems, alerts }: DashboardHomeProps) {
  const [showQuickTips, setShowQuickTips] = useState(true)

  const watchlistPercentage = (stats.watchlist.current / stats.watchlist.max) * 100
  const alertsPercentage = (stats.alerts.current / stats.alerts.max) * 100
  const apiUsagePercentage = (stats.apiUsage.current / stats.apiUsage.max) * 100
          ...
          <Card className="relative mb-8 border-l-4 border-blue-600 bg-blue-50">
            <CardContent className="p-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="mb-3 text-lg font-semibold text-gray-900">⚡ Quick Start Tips:</h2>
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="bg-white">
            <CardContent className="p-6">
              ...
              <div className="mb-2 text-sm font-semibold uppercase text-gray-600">Watchlist</div>
              ...
              <Progress value={watchlistPercentage} className="h-2 bg-gray-200" />
            </CardContent>
          </Card>
          ...
          <Card className="bg-white">
            <CardContent className="p-6">
              ...
              <div className="mb-2 text-sm font-semibold uppercase text-gray-600">Alerts</div>
              <div className="mb-1 text-4xl font-bold text-gray-900">
                {stats.alerts.current}/{stats.alerts.max}
              </div>
              <div className="mb-3 text-sm text-gray-500">active</div>
              <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200">
                <div className="h-full bg-green-600 transition-all" style={{ width: `${alertsPercentage}%` }} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white">
            <CardContent className="p-6">
              ...
              <div className="mb-2 text-sm font-semibold uppercase text-gray-600">API Usage</div>
              <div className="mb-1 text-4xl font-bold text-gray-900">
                {stats.apiUsage.current}/{stats.apiUsage.max}
              </div>
              <div className="mb-3 text-sm text-gray-500">req/hour</div>
              <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200">
                <div className="h-full bg-yellow-500 transition-all" style={{ width: `${apiUsagePercentage}%` }} />
              </div>
              {isApiHighUsage && (
                <div className="mt-2 text-xs text-orange-600 flex items-center gap-1">⚠️ High usage</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-white">
            <CardContent className="p-6">
              ...
              <div className="mb-2 text-sm font-semibold uppercase text-gray-600">Chart Views</div>
              <div className="mb-1 text-4xl font-bold text-gray-900">{stats.chartViews.current}</div>
          ...
          {watchlistItems.length > 0 ? (
            <div className="space-y-4">
              {watchlistItems.map((item, index) => (
                <Card key={index} className="bg-white transition-shadow hover:shadow-md">
                  <CardContent className="p-4">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      ...
                      <div className="flex-1 text-center lg:text-left">
                        <div className="text-2xl font-semibold text-gray-900">${item.currentPrice.toFixed(2)}</div>
                      </div>
            ...
            <Card className="border-2 border-dashed border-gray-300 bg-gray-50">
              <CardContent className="p-12 text-center">
                ...
                <p className="mb-4 text-sm text-gray-400">Add your first symbol to start monitoring</p>
          ...
          {alerts.length > 0 ? (
            <div className="space-y-4">
              {alerts.map((alert) => {
                ...
                const StatusIcon =
                  alert.status === "watching" ? Clock : alert.status === "triggered" ? CheckCircle2 : PauseCircle

                return (
                  <Card key={alert.id} className={`bg-white border-l-4 ${borderColor}`}>
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <p className="text-sm text-gray-600">
                            {alert.symbol} • {alert.timeframe} • Target: ${alert.targetPrice.toFixed(2)}
                          </p>
                          <p className="text-sm text-gray-700">
                            Current: ${alert.currentPrice.toFixed(2)} | Distance: ${alert.distance.toFixed(2)} (
                            {alert.distance >= 0 ? "+" : ""}
            ...
            <Card className="border-2 border-dashed border-gray-300 bg-gray-50">
              <CardContent className="p-12 text-center">
                <div className="mb-4 text-6xl opacity-50">🔔</div>
                <h3 className="mb-2 text-xl text-gray-500">You haven't created any alerts yet.</h3>
                <p className="mb-4 text-sm text-gray-400">Set up alerts to get notified of price movements</p>
                <Button className="bg-blue-600 text-white hover:bg-blue-700 rounded-lg px-6 py-3">
            ...
            <CardContent className="p-8">
              <div className="flex flex-col items-start gap-6 lg:flex-row lg:items-center">
                ...
                <div className="flex-1">
                  ...
                  <p className="mb-4 text-lg">Get 15 symbols, 9 timeframes, and 20 alerts for just $29/month</p>
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                    ...
                    <a href="#" className="text-white underline text-sm hover:no-underline">
                      See full comparison
                    </a>
                  </div>
                </div>
